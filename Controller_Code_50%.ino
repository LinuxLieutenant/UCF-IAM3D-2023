...
void loop() {
  ...
  */
  ...
  inflator1.write(map(readChannel(4,10,1000,0),10,1000,0,90));//4 is the top left dial (channel 5) pin 4 and pin 8
  inflator2.write(map(readChannel(4,10,1000,0),10,1000,0,90));
  
  thrust.write(map(readChannel(1,-1000,1000,0),-1000,1000,45,135));   //1 is right stick up/down (channel 2) pin 5
  
//  left.write(readChannel(3,-180,180,0));    //3 is left stick left/right (channel 4) pin 6
  
  right.write(map(readChannel(3,0,1000,0),0,1000,0,90));     //3 is left stick left/right (channel 4) pin 7
  
  //This if statement makes it so the left motor and the right motor are controlled by the same axis
  if (readChannel(3, -1000, 1000, 0)<-12){ 
    left.write(map(readChannel(3,-1000,-10,0), 0, -1000, 0, 90));
  }
  else {
    left.write(0);
  }
